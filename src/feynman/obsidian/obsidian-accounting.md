...
> 场景：用微信支付结账后，打开系统「快捷指令」截图识别，文字自动写入 `journals/bill.md`，再执行一条命令即可完成记账——全程无需手动输入金额。
...
```text
obsidian://advanced-uri?vault=漂泊者及其影子&commandid=obsidian-accounting%3Abill-import
```
...
> Advanced URI 格式：`obsidian://advanced-uri?vault=库名&commandid=obsidian-accounting%3A命令ID`
