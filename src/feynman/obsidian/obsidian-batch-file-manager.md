...
- ✅ **修复失效图片链接**：扫描后按完整文件名推荐新路径，预览确认一键修正（保留 `![[img.png|300]]` 尺寸参数）
...
- ✅ **Markdown 格式检测**：扫描任务列表/标题缺空格、未闭合代码块、列表空格缩进，预览后一键修复（修复后不再复报）
- ✅ **重命名笔记并更新链接**：右键重命名单个笔记，或批量查找替换重命名（支持正则），自动同步全库 `[文本](路径.md)` Markdown 链接
...
| 查找重复笔记 | 找出 iCloud 冲突命名（`a 1.md`）和内容完全相同的重复笔记 |
